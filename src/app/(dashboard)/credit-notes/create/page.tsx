...
export default function CreateCreditDebitNotePage() {
  ...
  const form = useForm<NoteForm>({
    initialValues: {
      noteNumber: '',
      noteType: 'CREDIT',
      invoiceTypeCode: '381', // Default to 381 for credit note
  ...
  // Update noteType when invoiceTypeCode changes
  useEffect(() => {
    const isCredit = form.values.invoiceTypeCode === '381'
    const newNoteType = isCredit ? 'CREDIT' : 'DEBIT'
    if (form.values.noteType !== newNoteType) {
      form.setFieldValue('noteType', newNoteType)
      // Reset reason when switching types
      form.setFieldValue('reason', '')
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.values.invoiceTypeCode])
  ...
  const selectedReasons = form.values.noteType === 'CREDIT' ? CREDIT_NOTE_REASONS : DEBIT_NOTE_REASONS
  const isCredit = form.values.noteType === 'CREDIT'

  return (
    <PageLayout
      title={isCredit ? "Create Credit Note" : "Create Debit Note"}
      subtitle={isCredit ? "Create a new credit note for CamInvoice" : "Create a new debit note for CamInvoice"}
    >
        ...
        <Stack gap="lg">
          ...
          <Card withBorder>
            <Stack gap="md">
              ...
              <Textarea
                label="Description"
                placeholder={isCredit ? "Detailed description of the reason for this credit note" : "Detailed description of the reason for this debit note"}
                rows={3}
                required
