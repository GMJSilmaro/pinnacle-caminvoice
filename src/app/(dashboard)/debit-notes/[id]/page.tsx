...
function buildTimeline(note?: any) {
  ...
  // 1. Debit Note Created (always completed)
  items.push({
    id: 'created',
    title: 'Debit Note Created',
    description: 'Debit note draft created',
  ...
  if (isSubmitted) {
    ...
    const validationDesc = isValidated
      ? `Debit note validated - Status: ${note.camInvoiceStatus}`
      : isRejected
        ? `Debit note rejected - Status: ${note.camInvoiceStatus}`
        : hasStatus
...
export default function DebitNoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    ;(async () => {
      try {
        const [noteRes, itemsRes] = await Promise.all([
          fetch(`/api/debit-notes/${id}`, { credentials: 'include' }),
          fetch(`/api/debit-notes/${id}/line-items`, { credentials: 'include' }),
        ])
        
        if (noteRes.ok) {
          const noteData = await noteRes.json()
          if (mounted && noteData.success) {
            setDebitNote(noteData.debitNote)
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/debit-notes/${id}/submit`, {
        method: 'POST',
        credentials: 'include',
      })
      const data = await res.json()
      if (res.ok && data.success) {
        // Reload the debit note
        const noteRes = await fetch(`/api/debit-notes/${id}`, { credentials: 'include' })
        if (noteRes.ok) {
          const noteData = await noteRes.json()
          if (noteData.success) {
            setDebitNote(noteData.debitNote)
        ...
        showNotification.success('Debit note has been submitted to CamInvoice successfully', 'Submission Successful')
      } else {
        // Show error details
        console.error('Debit note submission failed:', data)
        const errorMessage = data.error || 'Unknown error'
        const details = data.details ? ` ${typeof data.details === 'string' ? data.details : JSON.stringify(data.details)}` : ''
        showNotification.error(`${errorMessage}${details}`, 'Submission Failed')
      }
    } catch (error) {
      console.error('Failed to submit debit note:', error)
      showNotification.error(error instanceof Error ? error.message : 'Unknown error', 'Submission Error')
    } finally {
  ...
  if (loading) {
    return (
      <PageLayout title="Debit Note" subtitle="Loading...">
        <Card withBorder>
          <Text c="dimmed">Loading debit note...</Text>
        </Card>
      </PageLayout>
    )
  }
  ...
  if (!debitNote) {
    return (
      <PageLayout title="Debit Note" subtitle="Not Found">
        <Card withBorder>
          <Text c="red" fw={500}>Debit note not found</Text>
          <Button mt="md" variant="light" leftSection={<IconArrowLeft size={16} />} onClick={() => router.push('/debit-notes')}>
            Back to Debit Notes
          </Button>
        </Card>
      </PageLayout>
  ...
  const canSubmit = isDraft || debitNote.status === 'REJECTED'

  return (
    <PageLayout
      title={debitNote.debitNoteNumber}
      subtitle={`${debitNote.type === 'DEBIT_NOTE' ? 'Debit' : 'Credit'} Note - Created on ${new Date(debitNote.createdAt).toLocaleDateString()}`}
      badge={{
        text: debitNote.camInvoiceStatus || debitNote.status,
        color: debitNote.camInvoiceStatus 
          ? getCamInvoiceStatusColor(debitNote.camInvoiceStatus) 
          : getStatusColor(debitNote.status)
      }}
      actions={
        <>
          {canEdit && (
            <Button
              ...
              onClick={() => router.push(`/debit-notes/${id}/edit`)}
            >
              Edit
            </Button>
          )}
    ...
    >
      <Tabs value={activeTab} onChange={(v) => setActiveTab(v || 'overview')}>
          ...
          <Grid>
            <Grid.Col span={{ base: 12, md: 8 }}>
              <Stack gap="md">
                    ...
                    <Stack gap="md">
                      ...
                      <Grid>
                        <Grid.Col span={6}>
                          ...
                          <Text fw={500}>{debitNote.originalInvoice.invoiceNumber}</Text>
                        </Grid.Col>
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Issue Date</Text>
                          <Text fw={500}>{new Date(debitNote.originalInvoice.issueDate).toLocaleDateString()}</Text>
                        </Grid.Col>
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Amount</Text>
                          <Text fw={500}>{debitNote.originalInvoice.currency} {Number(debitNote.originalInvoice.totalAmount).toFixed(2)}</Text>
                        </Grid.Col>
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Status</Text>
                          <Badge color={getStatusColor(debitNote.originalInvoice.status)} variant="light">
                            {debitNote.originalInvoice.status}
                          </Badge>
                        </Grid.Col>
                        {debitNote.originalInvoice.camInvoiceUuid && (
                          <Grid.Col span={12}>
                        ...
                        <Grid.Col span={12}>
                          <Button
                            ...
                            onClick={() => router.push(`/invoices/${debitNote.originalInvoice.id}`)}
                            fullWidth
                          >
                ...
                {!debitNote.originalInvoice && debitNote.originalInvoiceId && (
                  <Alert color="orange" icon={<IconAlertCircle size={16} />}>
                    <Text fw={500}>Original Invoice Reference Missing</Text>
                    <Text size="sm" mt="xs">
                      This debit note references an invoice (ID: {debitNote.originalInvoiceId}) but the invoice details could not be loaded.
                      The debit note requires a valid invoice reference to be submitted to CamInvoice.
                    </Text>
                  </Alert>
                )}
                ...
                {!debitNote.originalInvoiceId && (
                  <Alert color="red" icon={<IconAlertCircle size={16} />}>
                    ...
                    <Text size="sm" mt="xs">
                      This debit note must be linked to an original invoice before it can be submitted to CamInvoice.
                      Please edit the debit note and link it to an invoice.
                    </Text>
                  </Alert>
                )}
                  ...
                  <Stack gap="md">
                    ...
                    <Grid>
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500}>{debitNote.customer?.name || '—'}</Text>
                      </Grid.Col>
                      <Grid.Col span={6}>
                        <Text size="sm" c="dimmed">Tax ID / Member ID</Text>
                        <Text fw={500}>{debitNote.customer?.taxId || '—'}</Text>
                      </Grid.Col>
                      {debitNote.customer?.registrationNumber && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">VATTIN</Text>
                          <Text fw={500}>{debitNote.customer.registrationNumber}</Text>
                        </Grid.Col>
                      )}
                      <Grid.Col span={12}>
                        <Text size="sm" c="dimmed">Email Address</Text>
                        <Text fw={500}>{debitNote.customer?.email || '—'}</Text>
                      </Grid.Col>
                      {debitNote.customer?.phone && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Phone Number</Text>
                          <Text fw={500}>{debitNote.customer.phone}</Text>
                        </Grid.Col>
                      )}
                      {debitNote.customer?.city && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">City</Text>
                          <Text fw={500}>{debitNote.customer.city}</Text>
                        </Grid.Col>
                      )}
                      {debitNote.customer?.country && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Country</Text>
                          <Text fw={500}>{debitNote.customer.country}</Text>
                        </Grid.Col>
                      )}
                      {debitNote.customer?.postalCode && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Postal Code</Text>
                          <Text fw={500}>{debitNote.customer.postalCode}</Text>
                        </Grid.Col>
                      )}
                      {debitNote.customer?.address && (
                        <Grid.Col span={12}>
                          <Text size="sm" c="dimmed">Address</Text>
                          <Text fw={500}>{debitNote.customer.address}</Text>
                        </Grid.Col>
                      )}
                    </Grid>
                  </Stack>
                        ...
                        <Table.Tbody>
                          {lineItems.map((item: any) => (
                            <Table.Tr key={item.id}>
                              ...
                              <Table.Td>{debitNote.currency} {Number(item.unitPrice).toFixed(2)}</Table.Td>
                              <Table.Td>{(Number(item.taxRate) * 100).toFixed(0)}%</Table.Td>
                              <Table.Td>{debitNote.currency} {Number(item.lineTotal).toFixed(2)}</Table.Td>
                            </Table.Tr>
                          ))}
                        </Table.Tbody>
                      </Table>
                ...
                {(debitNote.reason || debitNote.notes) && (
                  <Card withBorder>
                    <Stack gap="md">
                      ...
                      {debitNote.reason && (
                        <div>
                          ...
                          <Text fw={500}>{debitNote.reason}</Text>
                        </div>
                      )}
                      {debitNote.notes && (
                        <div>
                          <Text size="sm" c="dimmed">Notes</Text>
                          <Text>{debitNote.notes}</Text>
                        </div>
                      )}
                    </Stack>
                  </Card>
                  ...
                  <Stack gap="md">
                    ...
                    <Grid>
                      <Grid.Col span={6}>
                        ...
                        <Text fw={500}>{new Date(debitNote.createdAt).toLocaleString()}</Text>
                      </Grid.Col>
                      <Grid.Col span={6}>
                        <Text size="sm" c="dimmed">Updated At</Text>
                        <Text fw={500}>{new Date(debitNote.updatedAt).toLocaleString()}</Text>
                      </Grid.Col>
                      {debitNote.submittedAt && (
                        <Grid.Col span={6}>
                          <Text size="sm" c="dimmed">Submitted At</Text>
                          <Text fw={500}>{new Date(debitNote.submittedAt).toLocaleString()}</Text>
                        </Grid.Col>
                      )}
                    </Grid>
                  </Stack>
            ...
            <Grid.Col span={{ base: 12, md: 4 }}>
              <Stack gap="md">
                ...
                <Paper p="md" withBorder>
                  <Stack gap="sm">
                    <Title order={4}>Debit Note Summary</Title>
                    <Group justify="space-between">
                      <Text size="sm" c="dimmed">Issue Date:</Text>
                      <Text fw={500}>{new Date(debitNote.issueDate).toLocaleDateString()}</Text>
                    </Group>
                    <Group justify="space-between">
                      <Text size="sm" c="dimmed">Currency:</Text>
                      <Text fw={500}>{debitNote.currency}</Text>
                    </Group>
                    <Group justify="space-between">
                      <Text size="sm" c="dimmed">Status:</Text>
                      <Badge color={getStatusColor(debitNote.status)} variant="light">
                        {debitNote.status}
                      </Badge>
                    </Group>
                    <Divider />
                    <Group justify="space-between">
                      ...
                      <Text fw={500}>{debitNote.currency} {Number(debitNote.subtotal).toFixed(2)}</Text>
                    </Group>
                    <Group justify="space-between">
                      <Text size="sm" c="dimmed">Tax:</Text>
                      <Text fw={500}>{debitNote.currency} {Number(debitNote.taxAmount).toFixed(2)}</Text>
                    </Group>
                    <Divider />
                    <Group justify="space-between">
                      <Text fw={700} size="lg">Total:</Text>
                      <Text fw={700} size="lg" c="green">
                        +{debitNote.currency} {Number(debitNote.totalAmount).toFixed(2)}
                      </Text>
                    </Group>
                  </Stack>
                </Paper>
                ...
                {debitNote.camInvoiceUuid && (
                  <Paper p="md" withBorder>
                    <Stack gap="sm">
                      <Group justify="space-between" align="center">
                        <Title order={4}>CamInvoice Details</Title>
                        {debitNote.camInvoiceStatus && (
                          <Badge
                            color={getCamInvoiceStatusColor(debitNote.camInvoiceStatus)}
                            variant="light"
                            size="sm"
                          >
                            {debitNote.camInvoiceStatus}
                      ...
                      {debitNote.camInvoiceStatusUpdatedAt && (
                        <Group justify="space-between">
                          <Text size="sm" c="dimmed">Status Updated:</Text>
                          <Text size="sm" fw={500}>
                            {new Date(debitNote.camInvoiceStatusUpdatedAt).toLocaleString()}
                          </Text>
                        </Group>
                      )}
                      {debitNote.verificationUrl && (
                        <Button
                          variant="light"
                          size="sm"
                          leftSection={<IconExternalLink size={14} />}
                          component="a"
                          href={debitNote.verificationUrl.includes('localhost')
                            ? debitNote.verificationUrl.replace(/https?:\/\/localhost:\d+/, 'https://sandbox.e-invoice.gov.kh')
                            : debitNote.verificationUrl
                          }
                          target="_blank"
                          fullWidth
        ...
        <Tabs.Panel value="timeline" pt="md">
          <Card withBorder>
            <Stack gap="md">
              <Title order={4}>Debit Note Processing Timeline</Title>
              {(() => {
                const timelineItems = buildTimeline(debitNote)
                ...
                {debitNote.xmlContent && (
                  <Group gap="xs">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        navigator.clipboard.writeText(debitNote.xmlContent || '');
                      }}
                    >
                      Copy XML
                    </Button>
              ...
              {debitNote.xmlContent ? (
                ...
                >
                  <pre style={{
                    ...
                    {showFormattedXML ? formatXML(debitNote.xmlContent) : debitNote.xmlContent}
                  </pre>
                </Paper>
              ) : (
                <Paper p="xl" bg="gray.1" ta="center">
                  <Text c="dimmed" size="sm">No XML content available yet.</Text>
                  <Text c="dimmed" size="xs" mt="xs">XML will be generated when the debit note is submitted to CamInvoice.</Text>
                </Paper>
              )}
            </Stack>
          </Card>
          ...
          <Card withBorder>
            <Stack gap="md">
              ...
              {debitNote.camInvoiceUuid ? (
                ...
                <Alert color="orange" icon={<IconAlertCircle size={16} />}>
                  <Stack gap="sm">
                    <Text fw={500}>Debit note not submitted to CamInvoice yet</Text>
                    <Text size="sm">
                      The official PDF will be available after submitting the debit note to CamInvoice.
                      Please submit the debit note first to view the official PDF with QR code.
                    </Text>
                  </Stack>
                </Alert>
              )}
