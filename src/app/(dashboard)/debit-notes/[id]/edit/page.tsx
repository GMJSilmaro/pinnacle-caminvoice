...
export default function EditDebitNotePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load debit note data, customers, invoices, and line items
  useEffect(() => {
    if (!id) return
    let mounted = true
    ;(async () => {
      try {
        const [noteRes, customersRes, invoicesRes, itemsRes] = await Promise.all([
          fetch(`/api/debit-notes/${encodeURIComponent(id)}`, { credentials: 'include' }),
          fetch('/api/customers?pageSize=100', { credentials: 'include' }),
          fetch('/api/invoices?pageSize=100', { credentials: 'include' }),
          fetch(`/api/debit-notes/${encodeURIComponent(id)}/line-items`, { credentials: 'include' }),
        ])
  ...
  async function saveLineItems() {
    try {
      ...
      const res = await fetch(`/api/debit-notes/${encodeURIComponent(id)}/line-items`, { 
        method: 'PUT', 
  ...
  async function submitToCamInv() {
    try {
      ...
      await saveLineItems()
      const res = await fetch(`/api/debit-notes/${encodeURIComponent(id)}/submit`, { method: 'POST', credentials: 'include' })
      const body = await res.json().catch(() => ({}))
      if (!res.ok || !body?.success) throw new Error(body?.error || 'Submit failed')
      showNotification.success('Debit note submitted to CamInvoice successfully')
      router.push('/debit-notes')
    } catch (e: any) {
      console.error(e)
      showNotification.error(e.message || 'Failed to submit debit note')
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      const res = await fetch(`/api/debit-notes/${encodeURIComponent(id)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
          debitNoteNumber: values.noteNo,
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || 'Failed to update debit note')
      }

      showNotification.success('Debit note updated successfully')
      
      router.push(`/debit-notes/${id}`)
    } catch (error: any) {
      console.error('Failed to update debit note:', error)
      showNotification.error(error.message || 'Failed to update debit note')
    } finally {
      setIsSaving(false)
    }
  }
  ...
  if (loading) {
    return (
      <PageLayout title="Edit Debit Note" subtitle="Loading...">
        <Card withBorder>
          <Group justify="center" p="xl">
            <Loader size="lg" />
            <Text>Loading debit note data...</Text>
          </Group>
        </Card>
      </PageLayout>
    )
  ...
  if (!debitNote) {
    return (
      <PageLayout title="Edit Debit Note" subtitle="Not Found">
        <Card withBorder>
  ...
  return (
    <PageLayout
      ...
      subtitle={`Edit debit note ${debitNote.debitNoteNumber}`}
      actions={
        <Button
          variant="light"
          leftSection={<IconArrowLeft size={16} />}
          onClick={() => router.push(`/debit-notes/${id}`)}
        >
          Back to View
        </Button>
      }
        ...
        <Stack gap="xl">
          ...
          {debitNote.status !== 'DRAFT' && (
            <Alert color="orange" icon={<IconInfoCircle size={16} />}>
              <Text fw={500} mb="xs">Note Status: {debitNote.status}</Text>
              <Text size="sm">
                This debit note has been submitted to CamInvoice. Only line items can be edited.
              </Text>
            </Alert>
          )}
            ...
            <Stack gap="md">
              ...
              <Grid>
                <Grid.Col span={{ base: 12, md: 6 }}>
                  <TextInput
                    label="Note Number"
                    placeholder={form.values.noteNo || 'DN-2024-001'}
                    required
                    disabled={debitNote.status !== 'DRAFT'}
          ...
          <Card withBorder>
            <Stack gap="md">
              ...
              <Alert color="blue" icon={<IconInfoCircle size={16} />} mb="md">
                <Text size="sm">
                  <strong>Required:</strong> A debit note must reference an original invoice before it can be submitted to CamInvoice.
                </Text>
              </Alert>
          ...
          <Group justify="flex-end">
            <Button variant="default" onClick={() => router.push(`/debit-notes/${id}`)}>
              Cancel
            </Button>
            {debitNote.status === 'DRAFT' && (
              <>
