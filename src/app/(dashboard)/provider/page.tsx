...
import ProviderTokenMonitor from "../../../components/admin/ProviderTokenMonitor.client";
...
export default function ProviderPage() {
  ...
  // Allow child components to refresh stats on demand (e.g., after revoke)
  const refreshStats = async () => {
    try {
      ...
      console.error("Failed to refresh provider stats:", error);
    }
  };
    ...
    return (
      <Stack p="md">
        <Alert color="red" icon={<IconShieldCheck size={16} />}>
          ...
          <Text size="sm">
            You do not have permission to access the Service Provider
            Administration panel. Only users with the 'PROVIDER' role can access
            this area.
          </Text>
        </Alert>
      </Stack>
  ...
  const stickyContent = (
    <Stack gap="md">
      ...
      <Alert color="blue" icon={<IconInfoCircle size={16} />}>
        ...
        <Text size="sm">
          Welcome, {user.firstName} {user.lastName}. This administration panel
          is restricted to Service Provider personnel only. All actions
          performed here affect the entire CamInvoice integration for all
          tenants.
        </Text>
      </Alert>
      ...
      <Grid>
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
          <StatsCard
            ...
            value={
              statsLoading ? "..." : stats.acceptedInvoices.toLocaleString()
            }
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
          <StatsCard
            ...
            value={
              statsLoading ? "..." : stats.monthlyInvoices.toLocaleString()
            }
            icon={<IconChartBar size={20} />}
            iconColor="blue"
            subtitle="This month"
            trend={
              stats.monthlyInvoiceGrowth !== 0
                ? {
                    value: `${
                      stats.monthlyInvoiceGrowth > 0 ? "+" : ""
                    }${stats.monthlyInvoiceGrowth.toFixed(1)}%`,
        ...
        <Tabs defaultValue="provider" variant="outline">
          ...
          <Tabs.Panel value="provider" pt="md">
            <ProviderSetup
              ...
              onRevokeComplete={() => {
                setStats((prev) => ({ ...prev, providerConnected: false }));
                refreshStats();
              }}
            />
          </Tabs.Panel>
          ...
          <Tabs.Panel value="system" pt="md">
            <ContentCard title="System Settings">
              <Stack gap="md">
                <Alert color="blue" icon={<IconInfoCircle size={16} />}>
                  System configuration options for the CamInvoice integration
                  will be implemented here. This includes webhook settings, API
                  rate limits, and global system preferences.
                </Alert>
                <Group>
                  <Button variant="light">Configure Webhooks</Button>
                  <Button variant="light">API Settings</Button>
                  <Button variant="light">System Logs</Button>
                </Group>
              </Stack>
            </ContentCard>
          </Tabs.Panel>
          ...
          <Tabs.Panel value="monitoring" pt="md">
            <ContentCard title="Provider Access Token Monitoring">
