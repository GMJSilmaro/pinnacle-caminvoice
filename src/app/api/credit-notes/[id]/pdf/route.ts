...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const creditNote = await client.creditNote.findUnique({
      where: { id },
      select: {
        id: true,
    ...
    if (!creditNote) {
      return new NextResponse('Credit note not found', { status: 404 })
    }

    // Check if credit note has been submitted to CamInvoice
    if (!creditNote.camInvoiceUuid) {
      return new NextResponse('Credit note not submitted to CamInvoice yet', { status: 400 })
    }
    ...
    try {
      ...
      const pdfBuffer = await CaminvoiceApi.downloadDocumentPdf({
        ...
        documentId: creditNote.camInvoiceUuid,
      ...
      const lines = [
        ...
        `Customer: ${creditNote.customer?.name ?? 'Unknown'}`,
        `Status: Submitted to CamInvoice`,
        `Document ID: ${creditNote.camInvoiceUuid}`,
        '',
      ...
      return new NextResponse(Buffer.from(pdf), {
        ...
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `inline; filename="${creditNote.creditNoteNumber}-fallback.pdf"`
        }
      })
    }
  } catch (e) {
