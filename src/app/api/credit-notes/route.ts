...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('CreditNotes GET error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!creditNoteNumber || !issueDate || !currency || !customerId || subtotal == null || taxAmount == null || totalAmount == null) {
    ...
    const created = await client2.creditNote.create({
      data: ({
    ...
    await logAudit({
      ...
      entityId: created.id,
      description: `Created credit note ${created.creditNoteNumber}`,
      request,
    })

    return NextResponse.json({ success: true, creditNote: created })
  } catch (error) {
    console.error('CreditNotes POST error:', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
