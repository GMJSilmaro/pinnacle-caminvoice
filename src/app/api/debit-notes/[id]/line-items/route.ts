...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const note = await prisma.debitNote.findFirst({ where: { id, ...(user.role === 'PROVIDER' ? {} : { tenantId: user.tenantId! }) }, select: { id: true } })
    if (!note) return NextResponse.json({ error: 'Debit note not found' }, { status: 404 })

    const items = await prisma.debitNoteLineItem.findMany({ where: { debitNoteId: id }, orderBy: { id: 'asc' } })
    return NextResponse.json({ success: true, items })
  } catch (error: any) {
    console.error('GET debit note line items error', error)
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params
    const note = await prisma.debitNote.findFirst({ where: { id, ...(user.role === 'PROVIDER' ? {} : { tenantId: user.tenantId! }) }, select: { id: true, currency: true } })
    if (!note) return NextResponse.json({ error: 'Debit note not found' }, { status: 404 })
    ...
    await prisma.$transaction([
      prisma.debitNoteLineItem.deleteMany({ where: { debitNoteId: id } }),
      prisma.debitNoteLineItem.createMany({ data: rows.map((r) => ({ ...r, debitNoteId: id })) }),
    ])

    return NextResponse.json({ success: true })
  } catch (error: any) {
    console.error('PUT debit note line items error', error)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
