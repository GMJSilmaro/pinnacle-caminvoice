...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const debitNote = await client.debitNote.findUnique({
      where: { id },
      select: {
        id: true,
    ...
    if (!debitNote) {
      return new NextResponse('Debit note not found', { status: 404 })
    }

    // Check if debit note has been submitted to CamInvoice
    if (!debitNote.camInvoiceUuid) {
      return new NextResponse('Debit note not submitted to CamInvoice yet', { status: 400 })
    }
    ...
    try {
      ...
      const pdfBuffer = await CaminvoiceApi.downloadDocumentPdf({
        ...
        documentId: debitNote.camInvoiceUuid,
      ...
      return new NextResponse(pdfBuffer, {
        ...
        headers: {
          ...
          'Content-Disposition': `inline; filename="${debitNote.debitNoteNumber}.pdf"`,
      ...
      const lines = [
        ...
        `Customer: ${debitNote.customer?.name ?? 'Unknown'}`,
        `Status: Submitted to CamInvoice`,
        `Document ID: ${debitNote.camInvoiceUuid}`,
        '',
      ...
      return new NextResponse(Buffer.from(pdf), {
        ...
        headers: {
          'Content-Type': 'application/pdf',
          'Content-Disposition': `inline; filename="${debitNote.debitNoteNumber}-fallback.pdf"`
        }
      })
    }
  } catch (e) {
