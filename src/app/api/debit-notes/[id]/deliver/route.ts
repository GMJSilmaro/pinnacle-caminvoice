...
export async function POST(
  ...
  try {
    ...
    if (deliveryResult.success) {
      ...
      let syncResult = null
      try {
        // Get the debit note to check if it has a CamInvoice UUID
          ...
          setTimeout(async () => {
            try {
              await StatusSyncService.syncDocumentStatus({
                documentId: id,
                documentType: 'debit-note',
                camInvoiceUuid: debitNote.camInvoiceUuid!,
              })
              console.log(`Auto-sync triggered for debit note ${id} after delivery`)
            } catch (syncError) {
              console.error(`Auto-sync failed for debit note ${id}:`, syncError)
            }
          }, 2000) // 2 second delay
      ...
      return NextResponse.json({
        success: true,
        message: `Debit note delivered successfully via ${deliveryResult.method}`,
